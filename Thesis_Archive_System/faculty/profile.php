<?php
include("../config/db.php"); // starts session and connects DB
...
function handle_upload($fieldName, $uploadDir, $dbField, $conn, &$student, &$message)
{
    ...
    if (move_uploaded_file($tmp_name, $target_file)) {
        ...
        $res = mysqli_query($conn, "UPDATE users SET {$dbField}='{$escaped}' WHERE user_id={$user_id}");
        if (!$res) {
            error_log('DB update failed (faculty/profile.php) for user_id=' . $user_id . ': ' . mysqli_error($conn));
        }
...
if (isset($_POST['update_profile'])) {
    $updates = 0;
    // Attempt both uploads; handle_upload updates session and $student on success
    $before = [$student['profile_picture'] ?? null, $student['signature'] ?? null];
    handle_upload('profile_picture', __DIR__ . '/../uploads/profiles', 'profile_picture', $conn, $student, $message);
    if ($student['profile_picture'] !== ($before[0] ?? null)) $updates++;
    $before2 = $student['signature'] ?? null;
    handle_upload('signature', __DIR__ . '/../uploads/signatures', 'signature', $conn, $student, $message);
    if ($student['signature'] !== ($before2 ?? null)) $updates++;

    if ($updates > 0) {
        // Provide a simple flash message and redirect back to dashboard so the updated images appear there
